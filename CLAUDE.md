...
- **From the search:** the home page search also matches dish names, and a restaurant card lists its matching dishes. The card passes their ids in the router state (`searchMatches`). `RestaurantDetail` then scrolls from the top of the page to the first one (its middle at a third of the screen), lights them all up (`.menu-card-flash`) and clears `searchMatches`, so a reload or Back doesn't repeat it.
- The search term is kept in the home page's address (`/?q=`), replaced while typing, so browser Back and reloads keep the results. The input keeps its own state, because the router updates the address in a transition and an input bound to it would drop keystrokes. The card also passes `state.from`, which the restaurant page's back button links to.
...
- **Menu drag and drop uses only `@dnd-kit/core`, not `@dnd-kit/sortable`:** the sortable package treats "over a dish" as "take its place", so a dish dragged into another category just above a dish lands below it. `SortableMenu` instead works out the drop spot from how far the dragged row covers the rows and headers it passes (a neighbour makes way at two thirds, `SWAP_AT`; the category a dish goes to still follows the header middles) and reorders the list live, with its own slide animations. Keep the two in step: its folding and slide code relies on the list being `position: relative` (for `offsetTop`) and on the overlay rendering through a portal (the card's `backdrop-blur` breaks `position: fixed`).
