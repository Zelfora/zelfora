...
The portal has three tabs: `/partner` (orders: accept, move on, reject or cancel), `/partner/menu` (add, edit, reorder, mark as sold out and delete dishes) and `/partner/settings` (details, opening hours, photo). Above the tabs, a switch pauses and resumes orders. Each tab is a `Partner*` component in `components/`.
